 ...
 */
public class ChangePasswordAcivity extends BackActivity {
    ...
    private void confirmClicked(View v){
        ...
        if (!accountInfo.getPassword().equals(oldPasswordText.getText().toString().trim())){
            MyToast.makeText(this, "密码错误",Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (!newPassword.equals(passwordAgain)){
            MyToast.makeText(this, "两次输入密码不一致",Toast.LENGTH_SHORT).show();
            return;
        }
